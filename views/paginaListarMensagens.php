					...
					<tr>
						...
						<?php
							if ($contato->getResposta()->getDescricao() == "") {
								?><td colspan="1" style="text-align: center"><a href="paginaResponder.php?idContato=<?=$contato->getCodigo()?>&nome=<?=$contato->getNome()?>&email=<?=$contato->getEmail()?>&mensagem=<?=$contato->getMensagem()?>">Responder</a></td><?php
							} else {
								?><td colspan="1" style="text-align: center"><a href="<?php echo URL_BASE.'/controller/excluirResposta.php'?>?idResposta=<?=$contato->getResposta()->getCodigo()?>">Excluir resposta</a></td><?php
							}
						?>
					</tr>
				<?php
